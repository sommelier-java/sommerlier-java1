...
public class UserWineView {
    ...
    public void wineFiltering(){
        ...
        switch (num){
            case "1":
                ...
                WHILE : while(true){
                        System.out.println("필터링하실 번호를 입력해 주세요");
                ...
                printWineList(filteredWineList); //필터링된 와인 출력
            ...
            case "2":
                ...
                printWineList(filteredWineList);//필터링 된 와인 출력
                chooseWine();//필터링 된 와인에서 구매or장바구니 할 와인 선택
                break;
    ...
    //필터링 된 와인 출력
    public void printWineList(List<Wine> filteredWineList){
        if(filteredWineList==null) System.out.println("찾으시는 와인이 없습니다.");
        else filteredWineList.stream().forEach(d-> System.out.println(d)); //필터링 된 와인 출력
    }

    //필터링 된 와인에서 구매 or 장바구니 할 와인 선택 + 구매 or 장바구니 선택
    private void chooseWine(){
        System.out.println("원하시는 와인의 이름을 입력해 주세요");
        System.out.println("9. 뒤로가기");
        String wineName=input(">>");
        if(wineName.equals("9")) return;
